...
export default function DoctorCases() {
  ...
  TABS.forEach((t) => {
    counts[t] = t === 'All' ? myCases.length : myCases.filter((c) => c.status === TAB_STATUS_MAP[t]).length;
  });
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="page-subtitle">{myCases.length} cases total</p>
      </div>
              ...
              {filtered.map((c) => (
                <tr key={c.id} className="hover:bg-surface-100 transition-colors">
                  ...
                  <td className="table-cell">
                    <Link to={`/patient/${c.patientId}`} className="font-medium text-surface-800 hover:text-navy-700 hover:underline">{c.patientName}</Link>
                  </td>
                  <td className="table-cell text-surface-600 text-xs">{c.disease || '—'}</td>
