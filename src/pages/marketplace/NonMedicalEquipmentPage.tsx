...
export default function NonMedicalEquipmentPage() {
  ...
  // Filter strictly for NON_MEDICAL equipment
  const nonMedicalEquipment = useMemo(() => {
    return equipmentCatalog.filter((item) => item.category === 'NON_MEDICAL');
  }, [equipmentCatalog]);

  // Dynamic subcategories for non-medical equipment
  const availableSubcategories = useMemo(() => {
    const categories = new Set<string>();
    nonMedicalEquipment.forEach((item) => {
      if (item.subcategory) {
  ...
  const stats = useMemo(() => {
    const total = nonMedicalEquipment.length;
    const available = nonMedicalEquipment.filter((item) => item.availability === 'AVAILABLE').length;
    const sourcing = nonMedicalEquipment.filter((item) => item.availability === 'REQUEST_SOURCING').length;
    const unavailable = nonMedicalEquipment.filter((item) => item.availability === 'UNAVAILABLE').length;
    return { total, available, sourcing, unavailable };
  }, [nonMedicalEquipment]);

  // Search & Filter
  const filteredEquipment = useMemo(() => {
    const list = nonMedicalEquipment.filter((item) => {
    ...
    return list;
  }, [nonMedicalEquipment, selectedSubcategory, selectedAvailability, selectedProcurementMode, searchQuery, sortBy]);
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <section className="border-b border-[#E2E8F0] bg-[#EAF2F5] px-4 py-6 sm:px-6">
        <div className="mx-auto max-w-[1500px]">
          ...
          <nav className="mb-3.5 flex items-center gap-1.5 text-xs text-[#64748B]">
            <Link to="/marketplace" className="hover:text-[#245B70] hover:underline">
              Marketplace
            </Link>
            <ChevronRight className="h-3.5 w-3.5" />
            <span className="font-semibold text-[#112A28]">Non-Medical Equipment</span>
          </nav>

          {/* MAIN SEARCH BOX (MUDAH.MY MULTI-SEGMENT BAR) */}
          <div className="rounded-xl border border-[#D0E2EB] bg-white p-2 shadow-sm">
            <div className="flex flex-col gap-2 md:flex-row md:items-center">
              ...
              <div className="w-full md:w-[220px]">
                <select
                  ...
                  className="h-11 w-full rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-3 text-xs font-semibold text-[#112A28] outline-none focus:border-[#245B70] focus:bg-white"
                >
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search Non-Medical Facility Equipment, Power, IT, Clinic Furniture..."
                  className="h-11 w-full rounded-lg border border-[#E2E8F0] bg-white pl-10 pr-4 text-xs text-[#112A28] outline-none placeholder:text-[#94A3B8] focus:border-[#245B70]"
                />
              </div>
              ...
              <div className="w-full md:w-[200px]">
                <select
                  ...
                  className="h-11 w-full rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-3 text-xs font-medium text-[#112A28] outline-none focus:border-[#245B70] focus:bg-white"
                >
              ...
              <button
                type="button"
                className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-[#245B70] px-6 text-xs font-bold uppercase tracking-wider text-white shadow-sm transition-colors hover:bg-[#1C485A]"
              >
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 border-t border-[#F1F5F9] pt-2">
              <select
                ...
                className="h-8 rounded-md border border-[#E2E8F0] bg-white px-2.5 text-[11px] font-medium text-[#45645E] outline-none focus:border-[#245B70]"
              >
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="inline-flex h-8 items-center gap-1.5 rounded-md border border-[#CDE1E8] bg-[#F0F7FA] px-2.5 text-[11px] font-semibold text-[#245B70] hover:bg-[#E2EFF4]"
                >
              ...
              {isSuperOrMaster && (
                ...
                >
                  <Settings className="h-3 w-3 text-[#245B70]" />
                  Catalogue Admin
                </button>
              )}
            </div>
      ...
      <main className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6">
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-[17px] font-bold text-[#112A28]">
              Non-Medical & Facility Equipment with Total{' '}
              <span className="font-extrabold text-[#245B70]">{stats.total}</span> Available in Malaysia
            </h1>
            <p className="text-xs text-[#64748B]">
              Browse healthcare clinic vehicles, power generators, cold chain, and facility infrastructure.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="h-9 rounded-lg border border-[#E2E8F0] bg-white px-3 text-xs font-semibold text-[#112A28] outline-none focus:border-[#245B70]"
            >
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2">
          ...
          {availableSubcategories.map((sub) => {
            const count = nonMedicalEquipment.filter((i) => i.subcategory === sub).length;
            ...
            return (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
                  isSelected
                    ? 'border border-[#245B70] bg-[#F0F7FA] font-bold text-[#245B70]'
        ...
        {filteredEquipment.length === 0 ? (
          <div className="rounded-xl border border-[#E2E8F0] bg-white px-6 py-16 text-center shadow-sm">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-[#F0F7FA] text-[#245B70]">
              <Search className="h-6 w-6" />
            </div>
            <h3 className="mt-4 text-base font-bold text-[#112A28]">No non-medical equipment found</h3>
            ...
            <button
              type="button"
              onClick={handleResetFilters}
              className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-[#245B70] px-4 py-2 text-xs font-semibold text-white hover:bg-[#1C485A]"
            >
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredEquipment.map((item) => {
              ...
              return (
                <article
                  key={item.id}
                  className="group flex flex-col overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[#ADC7D4] hover:shadow-md"
                >
                  {/* CATEGORY PREVIEW HEADER */}
                  <div className="relative flex h-44 items-center justify-center border-b border-[#E2E8F0] bg-[#F8FAFC]">
                    <div className="flex h-20 w-20 items-center justify-center rounded-2xl border border-[#D0E2EB] bg-white text-[#245B70] shadow-sm transition-transform duration-200 group-hover:scale-105">
                    ...
                    <div className="absolute left-3 top-3">
                      <span className="rounded-md border border-sky-200 bg-sky-50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-sky-700">
                        Non-Medical
                      </span>
                    </div>
                  ...
                  <div className="flex flex-1 flex-col p-4">
                    <p className="text-[11px] font-bold uppercase tracking-wider text-[#245B70]">
                      {item.manufacturer}
                    </p>
                    ...
                    <div className="mt-3 min-h-[42px] space-y-1">
                      {item.keySpecifications.slice(0, 2).map((spec, index) => (
                        ...
                        >
                          <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-[#245B70]" />
                    ...
                    <div className="mt-3 flex items-center justify-between border-t border-[#F1F5F9] pt-2.5 text-[10px] text-[#64748B]">
                      ...
                      <span className="flex items-center gap-1 font-medium">
                        <Clock className="h-3 w-3 text-[#245B70]" />
                    ...
                    <div className="mt-3 flex gap-2">
                      <button
                        ...
                        className="flex-1 inline-flex h-8 items-center justify-center gap-1 rounded-lg border border-[#E2E8F0] bg-white px-2 text-xs font-semibold text-[#45645E] transition-colors hover:border-[#ADC7D4] hover:bg-[#F8FAFC] hover:text-[#245B70]"
                      >
                      ...
                      {canRequestQuotation && (
                        isAvailableForRfq ? (
                          <button
                            ...
                            className="flex-1 inline-flex h-8 items-center justify-center gap-1 rounded-lg bg-[#245B70] px-2 text-xs font-bold text-white transition-colors hover:bg-[#1C485A]"
                          >
                      ...
                      {isSuperOrMaster && (
                        <button
                          ...
                          className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[#D8E8E2] bg-[#F0F7FA] text-[#245B70] hover:bg-[#E2EFF4]"
